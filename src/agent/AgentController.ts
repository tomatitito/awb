...
export class AgentController {
  ...
  getState(): AgentPanelState {
    ...
    return {
      ...
      selectedTicketId:
        this.selectedTicket?.ticketId ?? latestRun.ticket.ticketId,
  ...
  async promptRun(runId: string, text: string): Promise<void> {
    ...
    if (record.run.status === "aborted")
      throw new Error(`Run ${runId} has already been aborted.`);
    ...
    this.emit({
      ...
      runId,
      entry:
        record.run.transcript.entries[
          record.run.transcript.entries.length - 1
        ]!,
    });
  ...
  private handleSessionEvent(runId: string, event: AgentSessionEvent): void {
    ...
    switch (event.type) {
      ...
      case "message_update": {
        ...
        record.run.transcript.entries.push(entry);
        record.run.updatedAt = this.now();
        record.run.transcript.updatedAt = record.run.updatedAt;
      ...
      case "tool_execution_start": {
        ...
        record.run.transcript.toolActivity.push(tool);
        record.run.updatedAt = this.now();
        record.run.transcript.updatedAt = record.run.updatedAt;
      ...
      case "tool_execution_end": {
        const tool = record.run.transcript.toolActivity.find(
  ...
  private toPanelStatus(
    status: AgentRunState["status"],
  ): AgentPanelState["status"] {
    switch (status) {
      case "queued":
      case "starting":
        return "connecting";
...
export function toLegacyPanelEvent(
  event: AgentRunEvent,
): AgentPanelEvent | undefined {
  switch (event.type) {
    case "run-created":
    case "run-updated":
      return {
        ...
        state: {
          ...
          isStreaming:
            event.run.status === "running" || event.run.status === "starting",
